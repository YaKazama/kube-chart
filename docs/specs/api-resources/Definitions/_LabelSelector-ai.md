...
新增命名模板 `definitions.labelSelector`，写入 `templates/api-resources/Definitions/_LabelSelector.tpl`。入参为 dict，由 `definitions.deploymentSpec` 透传，包含 `matchExpressions` 和 `matchLabels` 两个字段。按 K8s 官方 API 字段顺序（matchExpressions → matchLabels）渲染 LabelSelector 资源，不渲染 `status` 字段。
...
引用模板 `definitions.labelSelectorRequirement`。列表元素支持 string / 原生 object 两种类型，向下传递上下文统一为 dict 形态（包含 `key`、`operator`、`values` 字段），由下层模板做最终字段级校验与渲染。

#### 列表元素类型识别与规整

- **dict 类型**：原生 object 类型，直接传递，包括 `key`（必填）、`operator`（必填）、`values` 字段。
- **string 类型**：正则解析为 dict 后传递，包含 `key`、`operator`、`values` 字段。标签选择支持：
  - 基于等值：`=` / `==` / `!=`
  - 基于集合：`In` / `NotIn` / `Exists` / `DoesNotExist`
  - 基于原生定义：直接透传
...
- **正则定义**：集中存放在 `templates/base/_env.tpl` 的 `K8S.SELECTOR` 路径下：
  - `EQUALITY0`：等值选择器（`key = value` / `key == value` / `key != value`），归一为 `In` / `NotIn` 操作符，values 为单值列表。
  - `SET0`：基于集合选择器（`key In (v1, v2, ...)` / `key NotIn (...)`），操作符保留原值，values 通过 `SET_VALUES_SPLIT` 拆分。
  - `SET_EXISTS`：基于存在性选择器（`key` / `!key`），归一为 `Exists` / `DoesNotExist` 操作符，无 values 字段。

#### 回收处理

- 列表元素委托 `definitions.labelSelectorRequirement` 渲染后回收，逐项追加到结果列表。
- 回收时执行 `mustUniq | mustCompact` 去重去空。
- 元素渲染为空或非 map 时立即 fail。
...
- 上层（`definitions.deploymentSpec`）已通过 `base.labels` 合并默认值，模板入口已保证 `name` 等默认标签存在。
- 入参必须为非空 map，缺失或非 map 类型立即 fail。
- 通过 `base.field` 渲染为 `base.map`。

## 专属边界行为

- **必填项缺失**：`matchLabels` 缺失或 `base.get` 返回 `"null"` 时立即 fail 中断渲染。
- **类型非法**：`matchLabels` 传入非 map 类型时立即 fail。
- **元素类型非法**：`matchExpressions` 元素不是 string 或 map 类型时立即 fail。
- **不支持的选择器**：string 类型元素不能匹配 `EQUALITY0` / `SET0` / `SET_EXISTS` 任一正则时立即 fail。
- **空值检查**：`SET0` 选择器通过 `SET_VALUES_SPLIT` 拆分后的每个 value 必须非空，否则立即 fail。
- **回收校验**：每个元素经下层模板渲染后为空或非 map 时立即 fail。
- **list/map 类型 BUG 兼容**：
  - `matchExpressions` 需通过 `base.isFromYamlArrayError` 兼容 Helm 4.2.2 `fromYamlArray` 对非列表输入返回错误切片的 BUG。
  - `matchLabels` 需通过 `base.isFromYamlError` 兼容 Helm 4.2.2 `fromYaml` 对非 map 输入返回错误 map 的 BUG。
...
- 字段处理和渲染顺序必须严格对齐 K8s 官方 API 展示顺序（matchExpressions → matchLabels），形成"处理-渲染"单字段闭环。
- 必须使用 `base.get` 取值，禁止绕开手写取值。
- 渲染统一使用 `base.field`，禁止绕开。
- 正则集中管理（`K8S.SELECTOR` 系列位于 `templates/base/_env.tpl`），禁止模板中直接使用正则。
- 优先使用 `must` 系列函数。
