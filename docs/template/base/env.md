...
k8s:
  ...
  volume:
    ...
    mount: "^(\\S+)(?:\\s+(\\S+))(?:\\s+(\\S+))?(?:\\s+(true|false))?(?:\\s+(Disabled|IfPossible|Enabled))?(?:\\s+(Bidirectional|HostToContainer|None))?$"
    mountSubPathLike: "\\$\\([^)]+\\)"
  ...
  service:
    # format: [nodePort:]port[:targetPort][/protocol][@appProtocol][#name]
    ...
    endpoint:
      # format: port[/protocol][@appProtocol][#name]
      port: "^(0|[1-9]\\d{0,3}|[1-5]\\d{4}|6[0-4]\\d{3}|65[0-4]\\d{2}|655[0-2]\\d|6553[0-5])(?:\\/(tcp|TCP|udp|UDP|sctp|SCTP))?(?:@([\\w\\.-\\/]+))?(?:#([\\w-]+))?$"
      # node|zone name...
      #   name 最多 8 个
      hints: "^(node|zone)(?:\\s+((?:\\S+\\s*){1,8}))$"
net:
