{{- /*
  ...
  边界: 仅验证并序列化 map，不递归校验键和值的业务语义。

  返回值: YAML 字符串；非字典输入时中断渲染。
...
{{- /*
  ...
  入参: bool，或字面量字符串 "true"、"false"。

  边界: 不接受 yes/no、0/1 等其他布尔表示。

  返回值: 布尔字符串（"true" 或 "false"）；非法输入时中断渲染。
...
{{- /*
  ...
  入参: int、int64、float64 或整数字符串。

  边界: 不限制数值范围；范围校验由调用方或 base.int.range 负责。
...
{{- /*
  ...
  入参: 非负 int、int64、float64 或正整数字符串。

  边界: 只保证非负整数，不承担业务字段的上限校验。

  返回值: int（十进制）；非法输入时中断渲染。
...
{{- /*
  ...
  边界: 不按 Kubernetes 或业务字段规则限制字符串内容。

  返回值: 字符串；非法输入或 trim 后为空时中断渲染。
...
{{- /*
  ...
  边界: 仅验证并序列化 slice，不递归校验元素的业务语义。

  返回值: YAML 字符串；非切片输入时中断渲染。
