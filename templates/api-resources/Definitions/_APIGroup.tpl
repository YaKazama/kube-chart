...
{{- define "definitions.APIGroup" -}}
  ...
  {{- /* Step 5: preferredVersion (string, 可选): 正则匹配后转交 definitions.GroupVersionForDiscovery 渲染 */ -}}
  {{- $_preferredVersion := include "base.get" (list . "preferredVersion") | trim }}
  {{- if $_preferredVersion }}
    ...
    {{- end }}
    {{- $preferredVersionObj := include "definitions.GroupVersionForDiscovery" $_preferredVersion | fromYaml }}
  ...
  {{- /*
    Step 6: serverAddressByClientCIDRs (array, 可选): 逐项校验后渲染为对象列表
  ...
  */ -}}
  ...
  {{- if $_cidrsRaw }}
    ...
    {{- range $c := $cidrs }}
      ...
      {{- end }}
      {{- $cObj := include "definitions.ServerAddressByClientCIDR" $cStr | fromYaml }}
      {{- if not $cObj }}
  ...
  {{- /* Step 7: versions (array, 必填, 非空): 元素处理同 Step 6 走 base.get + fromYamlArray 路径 */ -}}
  ...
  {{- range $v := $versions }}
    ...
    {{- end }}
    {{- $vObj := include "definitions.GroupVersionForDiscovery" $vStr | fromYaml }}
    {{- if not $vObj }}
