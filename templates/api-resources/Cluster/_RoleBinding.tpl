{{- define "cluster.RoleBinding" -}}
  ...
  {{- if $roleRefVal }}
    ...
    {{- $val := dict "apiGroup" "rbac.authorization.k8s.io" "kind" "Role" }}
    {{- if and (eq $isNotSlice "true") (eq $isNotMap "true") }}
      {{- $val = mergeOverwrite $val (dict "name" $roleRefVal) }}
    {{- else if eq $isNotMap "false" }}
      {{- $val = mergeOverwrite $val (pick $roleRefVal "apiGroup" "kind" "name") }}
    {{- end }}
  ...
  {{- range $subjectsVal }}
    ...
    {{- else if kindIs "map" . }}
      {{- if empty (get . "kind") }}
        {{- fail "cluster.RoleBinding: kind cannot be empty" }}
      {{- end }}
