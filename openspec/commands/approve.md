...
- `draft.md`、全部 `plan/`
- 仅为复查目标冲突读取其他活动 change 的 draft frontmatter 与 `plan/spec.md` 的 `## 技术目标`
...
- 仅当 draft、plan、当前规格和适用规则不足以独立复核影响契约的目标事实时，以 [`../references/README.md`](../references/README.md) 路由，只读取与未决事实直接相关的本地参考资料；不得机械重读 `/sdd-plan` 用过的全部资料
...
- 通过时按变更文档规则创建或覆盖 `records/approval.md`，将状态改为 `approved`，并报告冻结范围。
