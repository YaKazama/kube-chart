...
本文件只提示 `/sdd-approve` 的检查范围，不是批准或验证证据。

- [ ] 初次检查和确认冻结时 draft frontmatter `status` 均为 `planned`，且 `draft-content-sha256` 与最近一次 plan 一致。
...
- [ ] 首次检查后保持 `planned`；仅用户显式 `confirm` 后冻结并进入 `approved`。
