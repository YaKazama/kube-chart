...
- `/sdd-new` 中的模板目标使用 `<define 名称>=<目标 tpl 文件>` 显式绑定；目标文件必须位于 `templates/` 下，命名空间、依赖域、目录和文件名必须与本节规则一致。
...
- 禁止创建未定义或仅占位的模板。
- 自顶向下实施时，父模板可以先写出已在批准材料中声明的子模板 `include` 占位引用；正式 `templates/` 中仍禁止创建返回空值或固定假数据的占位 `define`。子模板完成前不得使用临时替身判定父模板通过验证。
