...
- 规划直接渲染 Kubernetes API 字段的命名模板时，`plan/design.md` 必须按 API 字段顺序记录当前层负责的字段名称或路径、官方类型和功能；委托字段不得展开子模板内部结构。
- API 类型必须按官方字段表原样记录；Schema 约束与 Helm/Sprig 运行时类型在 design 的类型边界中另行说明，不得拼接为自定义类型名称。
- 实现直接渲染 Kubernetes API 字段的命名模板时，每个字段的中文注释必须写在 `define` 块内，并紧邻对应处理闭环之前；不得写入整体契约注释或与字段处理分离。
- 字段注释必须包含字段名称或路径、官方 API 类型和功能说明；不得使用 Helm/Sprig 运行时 `kind` 代替 API 类型。
