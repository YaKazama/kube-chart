...
本文件定义资源模板和共享结构模板调用核心命名模板的工程契约，不替代 [`openspec/specs/`](../specs/) 中已验证的可观察行为规格。用户变更触及核心能力的输出或失败行为时，必须按工作流建立或修改对应能力规格。

只允许调用 [`templates/base/`](../../templates/base/) 中真实存在或已由当前变更规格引入的命名模板。可以保留明确标记的预留扩展协议，用于约束后续能力；预留协议不是当前可调用实现，也不能替代能力规格和实际验证。
...
- 返回值是 YAML 字符串。bool 可直接比较；map/list 按下文 YAML 解析保护分别使用 `fromYaml`/`fromYamlArray`。
- 调用方不得对返回的 YAML 字符串精确比较 `"{}"`、`"[]"` 或 `"null"` 来判断集合空值；必须先恢复、排除解析错误并验证真实类型，再按字段生效条件直接将集合用于 `if`，或使用 `empty`、`len` 判断。
...
- 强制类型、集合隔离和 YAML 解析保护遵循 `base.get`；不得把 Helm 4.2.2 的解析错误 map 当作有效命中。
...
- map/list 字段不得使用 `base.field (list <key> <父上下文> "<结构子模板>")` 直接委托；必须先调用结构子模板并恢复、验证类型，再使用 `base.field (list <key> <已验证值> "base.map"/"base.slice")` 渲染。
- 指定允许值列表时，`base.field` 使用 `base.string` 渲染并在输出前校验枚举；不在列表中的值必须失败。
- 渲染结果为空时不输出字段；map、list 和容器环境变量以换行缩进形式输出，多行字符串使用 `|-` 块标量，其他值使用单行键值对。
- `base.field` 只接收 key、已归一化 value 和渲染参数，不持有来源、缺失状态或可选性上下文，因此不推断字段是否生效。调用方须在解析、排错和类型检查后按规格判断恢复集合；只有字段生效时才调用。缺失与 `null` 按被调用模板契约处理；`base.map`/`base.slice` 可将空集合渲染为 `{}`/`[]`，不得误认为通用渲染器会自动省略字段。
...
- 未经变更规格、正式模板实现和 Helm 验证，不得在当前规格、values、Schema 或用户文档中宣称某个具体 `*FieldRefs` 能力已经可用。
