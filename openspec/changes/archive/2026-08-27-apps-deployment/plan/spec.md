...
- capability：`apps-deployment`
- artifact-type：Helm 命名模板（Kubernetes 资源）
- template-name：`apps.deployment`
- target-path：`templates/api-resources/Apps/_Deployment.tpl`
- direct-dependencies：`definitions.objectMeta`、`apps.deploymentSpec`、`base.isFromYamlError`、`base.field`、`base.map`
